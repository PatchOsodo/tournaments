...
const FORMATS = [
  ...
  { id: 'double_elimination', icon: '⌘', name: 'Double elimination', desc: 'Two losses to exit, 8 teams max' },
  { id: 'group_stage',        icon: '⊞', name: 'Group stage',        desc: 'Groups → knockout' },
];

function suggestFormat(n) {
  if (n <= 5)                return 'round_robin';
  if (n <= 8)                return 'double_elimination';
  if ([16].includes(n))      return 'elimination';
  return 'group_stage';
}
...
/* -----------------------------------------------------------------------------
   7d. DOUBLE ELIMINATION

   STRUCTURE (8 teams example)
   ────────────────────────────
   Winners bracket (WB):
     WB R1: 4 matches  → 4 winners advance, 4 losers drop to LB R1
     WB R2: 2 matches (Semis) → 2 winners advance, 2 losers drop to LB R2
     WB R3: 1 match (WB Final) → 1 winner waits, 1 loser drops to LB Final

   Losers bracket (LB):
     LB R1: 2 matches — WB R1 losers play each other
     LB R2: 2 matches — LB R1 winners vs WB R2 losers
     LB R3: 1 match (LB Semis) — LB R2 winners
     LB Final: 1 match — LB R3 winner vs WB Final loser

   Grand Final:
     WB champion vs LB champion. Single game (no bracket reset).

   ROUND NUMBERING (stored in DB):
     Rounds 1..WBrounds          = Winners bracket
     Rounds WBrounds+1..WBrounds+LBrounds = Losers bracket
     Round WBrounds+LBrounds+1  = Grand Final

   MATCH TYPES stored in fixture:
     bracket_side: 'winners' | 'losers' | 'grand_final'
   ----------------------------------------------------------------------------- */
function genDoubleElimination(teams) {
  Logger.debug('genDoubleElimination', { count: teams.length });

  if (teams.length > 8) {
    throw new Error('Double elimination supports 8 teams or fewer');
  }

  // Pad to next power of 2
  let size = 1;
  while (size < teams.length) size *= 2;
  const slots = [...teams, ...Array(size - teams.length).fill('BYE')];

  // ── Winners bracket rounds
  const wbRounds = [];
  let wbMatchCount = size / 2;
  let roundNum = 1;
  ...
  for (let i = 0; i < size; i += 2) {
    ...
    wb1Matches.push({
      a, b, isBye,
      bracketSide     : 'winners',
      nextWinnerRound : roundNum + 1,
      nextWinnerMatch : Math.ceil(matchNum / 2),
      nextWinnerSlot  : matchNum % 2 === 1 ? 'home' : 'away',
      // Losers from WB R1 go to LB R1
      nextLoserRound  : null,  // filled in after LB is structured
      nextLoserMatch  : null,
      nextLoserSlot   : null,
    });
  }
  wbRounds.push({ roundNumber: roundNum, label: 'WB Round 1', matches: wb1Matches });
  roundNum++;

  // WB subsequent rounds
  wbMatchCount = wbMatchCount / 2;
  while (wbMatchCount >= 1) {
    const isLast  = wbMatchCount === 1;
    const label   = isLast ? 'WB Final' : `WB Round ${wbRounds.length + 1}`;
    const matches = [];
    for (let m = 1; m <= wbMatchCount; m++) {
      matches.push({
        a: 'TBD', b: 'TBD', isBye: false,
        bracketSide     : 'winners',
        nextWinnerRound : isLast ? null : roundNum + 1,
        nextWinnerMatch : isLast ? null : Math.ceil(m / 2),
        nextWinnerSlot  : m % 2 === 1 ? 'home' : 'away',
        nextLoserRound  : null,
        nextLoserMatch  : null,
        nextLoserSlot   : null,
      });
    }
    wbRounds.push({ roundNumber: roundNum, label, matches });
    roundNum++;
    wbMatchCount = wbMatchCount / 2;
  }
  ...
  //   Odd LB rounds: absorb WB losers
  //   Even LB rounds: internal LB winners play each other
  const lbRounds = [];
  let lbRoundNum = roundNum;

  // How many teams drop from each WB round:
  // WB R1: size/2 losers → size/4 LB matches in LB R1
  // WB R2: size/4 losers → size/4 LB matches in LB R2 (vs LB R1 winners)
  // WB R3 (WB Final): 1 loser → LB Final

  let lbMatchCount = size / 4;  // LB R1 match count

  // LB Round 1 — WB R1 losers vs each other
  const lb1Matches = [];
  for (let m = 1; m <= lbMatchCount; m++) {
    lb1Matches.push({
      a: 'TBD', b: 'TBD', isBye: false,
      bracketSide     : 'losers',
      nextWinnerRound : lbRoundNum + 1,
      nextWinnerMatch : m,
      nextWinnerSlot  : 'home',
      nextLoserRound  : null,
      nextLoserMatch  : null,
  ...
  lbRounds.push({ roundNumber: lbRoundNum, label: 'LB Round 1', matches: lb1Matches });
  lbRoundNum++;

  // Now pair off LB rounds for each subsequent WB round's losers
  for (let wbR = 2; wbR <= wbRoundCount; wbR++) {
    const isWbFinal     = wbR === wbRoundCount;
    const dropCount     = isWbFinal ? 1 : wbRounds[wbR - 1].matches.length;
    const prevLbWinners = lbRounds[lbRounds.length - 1].matches.length;
    const matchCount    = Math.max(dropCount, prevLbWinners);

    // Absorb round: LB winners vs WB droppers
    const absorbLabel   = isWbFinal ? 'LB Final' : `LB Round ${lbRounds.length + 1}`;
    const absorbMatches = [];
    for (let m = 1; m <= matchCount; m++) {
      const isLBFinal = isWbFinal && matchCount === 1;
      absorbMatches.push({
        a: 'TBD', b: 'TBD', isBye: false,
        bracketSide     : isLBFinal ? 'losers' : 'losers',
        nextWinnerRound : isLBFinal ? lbRoundNum + 1 : lbRoundNum + 1,
        nextWinnerMatch : isLBFinal ? 1 : Math.ceil(m / 2),
        nextWinnerSlot  : isLBFinal ? 'away' : (m % 2 === 1 ? 'home' : 'away'),
        nextLoserRound  : null,
        nextLoserMatch  : null,
        nextLoserSlot   : null,
      });
    }
    lbRounds.push({ roundNumber: lbRoundNum, label: absorbLabel, matches: absorbMatches });
    lbRoundNum++;

    // If not the WB Final drop, add an internal LB elimination round
    if (!isWbFinal && matchCount > 1) {
      const elim = Math.ceil(matchCount / 2);
      const elimLabel   = `LB Round ${lbRounds.length + 1}`;
      const elimMatches = [];
      for (let m = 1; m <= elim; m++) {
        elimMatches.push({
          a: 'TBD', b: 'TBD', isBye: false,
          bracketSide     : 'losers',
          nextWinnerRound : lbRoundNum + 1,
          nextWinnerMatch : Math.ceil(m / 2),
          nextWinnerSlot  : m % 2 === 1 ? 'home' : 'away',
          nextLoserRound  : null,
          nextLoserMatch  : null,
          nextLoserSlot   : null,
      ...
      lbRounds.push({ roundNumber: lbRoundNum, label: elimLabel, matches: elimMatches });
      lbRoundNum++;
    }
  }

  // ── Grand Final
  const grandFinalRound = lbRoundNum;
  const grandFinal = [{
  ...
  // ── Back-fill WB loser destinations into WB rounds
  // WB R1 losers → LB R1, distributed across LB R1 matches
  const lb1Round = lbRounds[0];
  wbRounds[0].matches.forEach((m, i) => {
    m.nextLoserRound = lb1Round.roundNumber;
    m.nextLoserMatch = Math.floor(i / 2) + 1;
    m.nextLoserSlot  = i % 2 === 0 ? 'away' : 'home';  // WB losers fill away slots
  });

  // WB R2+ losers → corresponding absorb LB rounds (every other LB round)
  // LB absorb rounds are at indices 1, 3, 5... in lbRounds
  let absorbIdx = 1;
  for (let wbR = 2; wbR <= wbRoundCount; wbR++) {
    const wbRound   = wbRounds[wbR - 1];
    const lbAbsorb  = lbRounds[absorbIdx];
    if (!lbAbsorb) break;
    wbRound.matches.forEach((m, i) => {
      m.nextLoserRound = lbAbsorb.roundNumber;
      m.nextLoserMatch = i + 1;
      m.nextLoserSlot  = 'away';  // WB droppers fill away slots in absorb rounds
    });
    absorbIdx += 2;
  }

  const allRounds    = [...wbRounds, ...lbRounds, ...grandFinal];
  const totalMatches = allRounds.reduce((s, r) =>
    s + r.matches.filter(m => !m.isBye).length, 0);

  Logger.info('genDoubleElimination done', {
    teams        : teams.length,
    size,
    wbRounds     : wbRounds.length,
    lbRounds     : lbRounds.length,
    grandFinal   : grandFinalRound,
    totalMatches,
  });
...
const App = {
  ...
  _renderFormatGrid() {
    ...
    // Disable double elimination for more than 8 teams
    if (n > 8 && State.setupData.format === 'double_elimination') {
      State.setupData.format = suggestFormat(n);
    }
    ...
    el.innerHTML = FORMATS.map(f => {
      ...
      const isDisabled = f.id === 'double_elimination' && n > 8;
      return `<div class="format-card ${isSel ? 'selected' : ''} ${isDisabled ? 'disabled' : ''}"
                   onclick="${isDisabled ? '' : `App.selectFormat('${f.id}')`}"
                   title="${isDisabled ? 'Double elimination is only available for 8 teams or fewer' : ''}">
        <div class="fmt-icon">${f.icon}</div>
        <div class="fmt-name">${f.name} ${isSug ? '<span class="badge-teal">suggested</span>' : ''}</div>
        <div class="fmt-desc">${isDisabled ? 'Available for 8 teams or fewer' : f.desc}</div>
      </div>`;
    }).join('');
    ...
    if (tip) {
      const tips = {
        ...
        double_elimination : `Double elimination for ${n} teams — one loss drops to losers bracket, two losses and you're out.`,
