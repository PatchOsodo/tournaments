 ...
 * CHANGES FROM v3
 * ---------------
 * 1. genElimination() — complete rewrite using a fixed-slot seed tree.
 *    Byes live only in round 1 as explicit match records. A bye match is
 *    immediately marked completed and its winner auto-placed into round 2
 *    during persistence, so later rounds always have the correct number of
 *    real teams. 6 teams → R1: 4 matches (2 real, 2 byes) → R2: 2 real
 *    matches (Semis) → R3: 1 match (Final). No ghost TBD slots.
 *
 * 2. genGroupStage() — progression now uses wins first, then point
 *    differential as tiebreaker (NBA-style). The advancers fed into the
 *    knockout bracket are the correctly ranked top-2 from each group.
 *
 * 3. DB.advanceWinnerElimination() — slot mapping updated to match the new
 *    fixed-slot scheme. Odd match numbers fill home_team, even fill away_team
 *    in the parent match of the next round.
 *
 * 4. _persistFixtures() — bye matches in elimination rounds are persisted
 *    as is_bye=true, status='completed', with the real team in home_team and
 *    no away_team. After saving, the bye winner is immediately placed into
 *    the next-round fixture so it appears correctly from the first load.
 *
 * TABLE OF CONTENTS
 * -----------------
 * 1.  Configuration
 ...
 *       7a. Round Robin  (unchanged)
 *       7b. Single Elimination  ← REWRITTEN
 *       7c. Group Stage         ← PROGRESSION LOGIC UPDATED
 * 8.  Database Layer (DB)
 *       advanceWinnerElimination  ← SLOT MAPPING UPDATED
 *       clearAdvancedWinner
 * 9.  App Controller
 *       9a. Initialisation
 *       9b. Home Screen
 *       9c. Setup Screen
 *       9d. Names Screen
 *       9e. Fixture Generation & Persistence  ← BYE SEEDING UPDATED
 *       9f. Fixtures Screen / Render Helpers  ← NBA bracket renderer
...
/* -----------------------------------------------------------------------------
   7b. SINGLE ELIMINATION — fixed-slot seed tree  ← REWRITTEN v4
   ...
   Round 2 (Semifinals):
     ...
     M2: Team5    vs Team6       → winner to Final away  ← correctly 2 teams
...
function genElimination(teams) {
  ...
  // This gives top seeds byes (they sit in the upper half, byes fill lower)
  ...
  for (let i = 0; i < size; i += 2) {
    const a     = slots[i];
    const b     = slots[i + 1];
    const isBye = b === 'BYE'; // a is never BYE since real teams fill first
    round1Matches.push({
      a,
      b,
      isBye,
      // Pre-compute next-round slot for persistence
      nextRound       : 2,
      nextMatchNumber : Math.floor(i / 2) + 1,  // 1-indexed
      nextSlot        : i % 4 === 0 ? 'home' : 'away',
    });
  }
...
/* -----------------------------------------------------------------------------
   7c. GROUP STAGE  ← PROGRESSION LOGIC UPDATED v4
...
/**
 * Compute live group standings from actual fixture results.
 *
 ...
 * @returns {Array} sorted array of standing objects
 */
/**
...
function _computeGroupStandings(fixtures, teams, groupName) {
  ...
  // Derive team IDs directly from the fixture records.
  // This is robust regardless of whether team.group_name is populated.
  // PocketBase SDK: after expand, relation fields still hold the raw ID string.
  // Guard against edge case where field holds the expanded object.
  const resolveId = (val) => {
    if (!val) return null;
...
const DB = {
   ...
   * next-round fixture.
   *
   * SLOT MAPPING (matches genElimination's nextSlot field):
  ...
  async seedKnockoutFromGroups(tournamentId, allTeams) {
    ...
    // Always re-fetch — ensures the fixture that triggered this call is
    // already reflected as completed before we check or compute standings.
    ...
    // Internal completion guard — only seed if ALL group fixtures are done
    ...
    // Get distinct group names from fixture data (not from team records)
    const groupNames = [...new Set(groupFxAll.map(f => f.group_name))].sort();
    Logger.info('Groups found', { groupNames });

    // Rank top-2 per group — _computeGroupStandings derives teams from fixtures,
    // so this works even if the group_name field on team records is missing.
    ...
    // Abort if any advancer is missing a teamId
    ...
    // Find and seed the first knockout round
...
const App = {
   ...
   *
   * KEY CHANGE v4: For elimination round 1, bye matches are saved with
  ...
  async _persistFixtures(tournamentId, generated, teamMap) {
    ...
    if (generated.type === 'elimination') {
      ...
      // Knockout round placeholders (teams TBD until groups finish)
      for (const round of generated.knockout.rounds) {
        for (let mi = 0; mi < round.matches.length; mi++) {
          const m = round.matches[mi];
          await DB.createFixture({
            tournament   : tournamentId,
            round        : roundOffset + round.roundNumber,
            match_number : mi + 1,
            round_label  : round.label,
            home_team    : null,
            away_team    : null,
   ...
   *   Preserving this prevents the tab resetting to 0 on every result save.
   */
  _renderFixturesScreen(activeTab = 0) {
    ...
    // Activate the requested tab — clamp to valid range
   ...
   *
   * Renders rounds as vertical columns with connector lines between
   * matches that feed into each other — similar to how ESPN or the NBA
   * display their playoff bracket.
   *
   * Structure:
   *   [R1 M1] ─┐
   *             ├─ [R2 M1] ─┐
   ...
   *   [R1 M4] ─┘
   *
   * The connector lines are rendered using SVG overlaid on the bracket columns.
   */
  _renderNbaBracket(fixtures) {
    ...
    const CARD_H    = 68;   // height of one match card (2 teams × 34px)
    const CARD_GAP  = 12;   // gap between cards in the same round
    const COL_W     = 180;  // column width
    const COL_GAP   = 40;   // gap between columns (where connectors live)
    const PADDING_V = 20;   // top/bottom padding

    // Calculate how many cards are in round 1 to set canvas height
    const r1Count   = rounds[roundNums[0]].length;
    const canvasH   = r1Count * CARD_H + (r1Count - 1) * CARD_GAP + PADDING_V * 2;

    /**
     * Calculate the vertical centre of a card at position `index` in a round
     * that has `total` cards, within the canvas height.
     */
    ...
    for (let ci = 0; ci < roundNums.length - 1; ci++) {
      ...
      const colLeft   = (COL_W + COL_GAP) * (ci + 1) - COL_GAP;  // start x of gap
      ...
      for (let mi = 0; mi < thisRound.length; mi++) {
        ...
        // Horizontal line from card edge to midpoint
        svgLines += `<line x1="${colLeft}" y1="${fromY}" x2="${midX}" y2="${fromY}"
                           stroke="var(--border-mid)" stroke-width="1.5" />`;
        // Vertical line from this match to sibling level
  ...
  async saveResult() {
    ...
    // Capture the active tab index BEFORE doing anything so we can restore it
    const activeTabIdx = (() => {
    ...
    try {
      ...
      // Determine whether this is a bracket match that requires winner advancement.
      // This covers:
      //   - Pure elimination tournaments (every match is a bracket match)
      //   - Group stage knockout fixtures (no group_name = knockout round)
      // Group stage GROUP fixtures must NOT advance — they feed into seeding instead.
      ...
      // Step 1: clear old advancement if editing a bracket result
      ...
      // Step 3: advance winner to the next bracket round.
      // For group stage knockout fixtures this works identically to elimination —
      // match N in round R feeds into slot ceil(N/2) of round R+1.
      if (isBracketMatch) {
      ...
      // Step 4: group stage — attempt knockout seeding on every result save.
      // seedKnockoutFromGroups checks internally whether all groups are done
      // and returns false (no-op) if not ready yet, so safe to call every time.
      // This removes the intermediate checkFx fetch and groupDone logic that
      // previously risked mis-evaluating completion state.
      ...
      // Step 5: single re-fetch AFTER all DB writes are done
      State.fixtures = await DB.getFixtures(fixture.tournament);

      // Step 6: tournament completion check
      ...
      // Step 8: determine which tab to show after re-render
      // If group stage just finished, auto-jump to Knockout tab (index 2)
      // so the user immediately sees the seeded bracket
...
document.addEventListener('DOMContentLoaded', () => {
  Logger.info('DOM ready — booting Tournament Manager v4.1.0');
