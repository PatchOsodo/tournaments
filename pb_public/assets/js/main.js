...
/* ── ORGANISE EVENTS MODAL ───────────────────────────────────────────── */
  ...
  async openOrganiseModal() {
    ...
    try {
      ...
      // Build datalist of known event names for autocomplete
      const datalistId = 'organise-event-suggestions';
      const datalistHtml = `<datalist id="${datalistId}">
        ${existingEvents.map(e => `<option value="${escHtml(e)}">`).join('')}
      </datalist>`;

      // Render one row per tournament
      const rows = tournaments.map(t => {
        const currentEvent = escHtml(t.event_name || '');
        const formatText   = t.format.replace(/_/g, ' ');
        return `
          <div style="
          ...
          ">
            <div>
              ...
              <div style="font-size:11px;color:var(--text-tertiary);margin-top:2px;">
                ${formatText} · ${new Date(t.created).toLocaleDateString()}
              </div>
            </div>
            <input
              type="text"
      ...
      list.innerHTML = datalistHtml + rows;

    } catch (e) {
      Logger.error('openOrganiseModal failed', { error: e.message });
  ...
  async saveOrganise() {
    ...
    try {
      ...
      for (const input of inputs) {
        ...
        const current = await pb.collection('tournaments').getOne(tournamentId, {
        ...
        if (newEvent !== currentEvent) {
          ...
          Logger.info('saveOrganise: updated event_name', {
            tournamentId,
            from : currentEvent ?? '(none)',
            to   : newEvent    ?? '(none)',
          });
          changeCount++;
        }
      ...
      Logger.info('saveOrganise: complete', { changeCount });
      document.getElementById('organise-overlay').style.display = 'none';

      // Reload home screen to show new groupings
      await App.loadTournaments();
      ...
      Logger.error('saveOrganise failed', { error: e.message });
      errEl.textContent    = `Save failed: ${e.message}`;
